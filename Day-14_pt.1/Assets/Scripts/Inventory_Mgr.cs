...
public class Inventory_Mgr : MonoBehaviour
{
    ...
    [Header("--- ShowNickName ---")]
    public InputField NickInputField;  //별명 입력 상자
    bool bEnter = false;
    ...
    void Start()
    {
        ...
        if (LvUp_Btn != null)
            LvUp_Btn.onClick.AddListener(LvUpBtnClick);  //레벨 강화 버튼

        if (GdUp_Btn != null)
            GdUp_Btn.onClick.AddListener(GdUpBtnClick);  //등급 강화 버튼
    ...
    void Update()
    {
        ...
        //{
        //    Debug.Log(ItName_Ifd.isFocused); //Enter 키가 눌려지면서 포커스를 잃게 된다.
        //}

        //닉네임 변경 입력 상자 OnOff
        if(Input.GetKeyDown(KeyCode.Return) == true)
        {
            //if (ItName_Ifd.isFocused == true) //아이템 입력상자에 포커스가 가 있으면...
            //    return;  //아이템 입력상자에 입력하기 위한 상황이면...
            ...
            if(bEnter == true)
            {
                ...
                NickInputField.ActivateInputField();    //<-- 커서를 인풋필드로 이동시켜 줌
            }
            else
            {
    ...
    void FindBtnClick()
    {
        ...
        float a_LvRate = (31 - a_LvStep) / 30.0f * 100.0f;  //1 ~ 30레벨이 최고 레벨
        if (m_LvRateText != null)
            m_LvRateText.text = "확률" + (int)a_LvRate + "%";

        int a_GdStep = m_ItemList[m_FIndex].m_Grade;
        float a_GdRate = a_GdStep / 7.0f * 100.0f;      //7 ~ 1등급 최고 등급
    ...
    void SellItem(float a_BosangRate = 0.8f)
    {
        if (m_FIndex < 0 || m_ItemList.Count <= m_FIndex)
            return;

        int a_CacCost = (int)(m_ItemList[m_FIndex].m_Price * a_BosangRate);
        g_GameGold += a_CacCost;
        if (0.8f <= a_BosangRate)
    ...
    void LvUpBtnClick()
    {
        ...
        {
            ShowMessage("<color=#ff8684>이미 최고 레벨입니다.</color>");
            return;
        }

        int a_Step = m_ItemList[m_FIndex].m_Level;
        int a_Chance = Random.Range(2, 32);     // 2 ~ 31  //30개
        if(a_Step < a_Chance)  // 1레벨일 때는 100%, 2레벨 90%, ... 29레벨일 때는 0.x%
        {
            m_ItemList[m_FIndex].m_Level++;
            ShowMessage("<color=#00ff00>레벨업에 성공했습니다. +1레벨 상승</color>");
        ...
        }//if(a_Step < a_Chance)  //레벨업 성공
        else  //레벨업에 실패 했을 때
        {
            CItem a_FNode = m_ItemList[m_FIndex];
            string a_StrBuff = "<color=#00ff00>레벨업 실패 ㅠㅠ; " + a_FNode.m_ItemUId + "번 " +
                        a_FNode.m_Name + "(이)가 가루가 되었습니다." + "</color>";
    ...
    void GdUpBtnClick()
    {
        ...
        {
            ShowMessage("<color=#ff8684>이미 강화 단계가 최대치입니다.</color>");
            return;
        }

        int a_Step = m_ItemList[m_FIndex].m_Grade;
        int a_Chance = Random.Range(0, 7);  // 0 ~ 6
        if(a_Chance < a_Step) //7등급일 때는 100%  6등급일 때는 85% ... 20%
        {  //성공시
            m_ItemList[m_FIndex].m_Grade--;

            ShowMessage("<color=#00ff00>등급업에 성공했습니다. 1등급 상승</color>");
            m_ItemList[m_FIndex].m_Price += (7 - m_ItemList[m_FIndex].m_Grade) * 1000;
        ...
        {
            CItem a_FNode = m_ItemList[m_FIndex];
            string a_StrBuff = "<color=#00ff00>등급업 실패 ㅠㅠ; " + a_FNode.m_ItemUId + "번 " +
    ...
    void ChangeNickName()
    {
        ...
        if(a_IsFail == true)
        {
            ShowMessage("닉네임은 2글자 이상 10글자 이하로 작성해 주세요.");
            return;
        }
